...
def load_sample_analysis_snapshot() -> tuple[pd.DataFrame, object] | None:
    if not AD_SAMPLE_PATH.exists():
        return None
    raw = load_ad_file(AD_SAMPLE_PATH)
...
def render_sample_analysis_preview(view_mode: str) -> None:
    ...
    if st.button("체험 데이터 분석 실행", use_container_width=True, key="sample_analysis_preview_button"):
        output_dir = BASE_DIR / "data" / "reports" / "sample_streamlit"
        result = run_pipeline_safely(AD_SAMPLE_PATH, output_dir, "광고주 A")
        if result is not None:
            render_analysis_result(result, view_mode, advertiser="광고주 A")

    st.caption("체험 데이터는 상위 50행만 미리보기로 표시합니다.")
    try:
        sample = pd.read_csv(AD_SAMPLE_PATH)
