...
- 보고서 센터: Excel / Markdown / TXT 데모 보고서 모두 표시 성공
...
배포 재발 방지를 위해 `.github/workflows/ci.yml`을 원격 저장소에 추가했다.

자동 검사 항목:

- Python 소스 컴파일
- 안전 데모 데이터 필수 컬럼과 행 수
- 온라인 체험용 파일 존재
- 데모 보고서 파일 존재
- 데모 데이터의 기존 분석 파이프라인 통과

첫 실행은 기존 테스트가 GitHub에서 제외된 `data/raw/sample_ads.csv`를 참조해 실패했다. 테스트 입력을 안전한 가짜 데이터 `samples/demo_data/campaignpulse_demo_ads.csv`로 변경했다.
...
- 온라인 데모 빈 화면 문제: 해결
- 예시 데이터 표시: 가능
- 보고서 센터 데모 파일 표시: Excel / Markdown / TXT 모두 가능
- GitHub Actions 자동 검증: 성공
- 실제 광고주 데이터 노출 위험: 낮음
- 앱 코드 완전 동기화: 미완료
